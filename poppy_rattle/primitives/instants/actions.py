...
class Instant_Actions(object):
	...
	def hand_wave(self,sec=None, rest=0.5):
		...
		self.robot.r_shoulder_y.goal_position = shoulder_y0self.robot.r_shoulder_x.goal_position = shoulder_x0
		...
		while (True):
			...
			if sec is not None:
				...
				try:
					...
					self.data.app()
					self.data.TIME.append(t1-t0)
					self.robot.r_shoulder_x.goal_position = shoulder_x1self.data.app()
					self.data.TIME.append(t1-t0)
					self.robot.r_arm_z.goal_position = arm_1
					self.data.app()
					self.data.TIME.append(t1-t0)
